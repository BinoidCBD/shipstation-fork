...
- Main entry points (at the repo root, not nested under `ask_self/`):
  - `ask_self_ingest.py`
  - `ask_self_query.py`
  - `ask_self_harness.py`
  - `ask_self_system_instructions.json`
- Harness templates (also at repo root):
...
- `pyproject.toml`, `setup.py`, `requirements.txt`, or a clearly Python-first repo → derive from `ask_self_harness.json`
...
- **Portable**: the **query** wrapper injects `--db-path "$REPO_ROOT/ask_self/index/<repo>.sqlite"` so a fresh clone can query immediately with no ingest. Users override by passing their own `--db-path` (argparse is last-wins). The **ingest** wrapper writes to the same path by default (e.g. via `--db-path` or by setting `db_filename` in the harness to the portable path).
...
4. Confirm relevant CLI flags and runtime requirements:
   - Ingest: `--harness-config`, `--mode`, `--no-prs`, `--no-register`, `--no-architecture-md`, `--shared-index`, `--cache-path`, `--no-cache`, `--events-file`, `--no-events`, `--dashboard`, `--embed-rpm`, `--embed-tpm`
   - Query: `--harness-config`, `--db-path`, `--retrieval-only`, `--local-only`, `--json`, `--targets`, `--all-targets`
...
- Ingest wrapper writes to the same path (use `--db-path` or pin `db_filename` in the harness)
...
6. Add wrapper scripts and local system instructions. Portable mode: query wrapper injects `--db-path` to the committed DB.
7. Update `.gitignore` per the mode (ignore sidecars always; ignore `ask_self/index/*.sqlite` only in local-only mode).
8. Update README, including a mode-appropriate setup section and (portable) the "last ingested" line.
9. Update `AGENTS.md` and any existing parallel agent files.
10. Run a smoke test. For portable mode, also confirm `du -sh ask_self/index/` is reasonable and set up Git LFS if > ~100 MB before committing.
